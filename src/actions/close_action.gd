...
func _init(p_actor: Monster, dir: Vector2i) -> void:
	super(p_actor)
	action_cost = ActionBudget.Cost.FREE
	direction = dir
...
func _execute(map: Map, result: ActionResult) -> bool:
	...
	if not obstacle:
		result.message = "There is nothing there to close."
		return false
	elif obstacle.type == Obstacle.Type.DOOR_CLOSED:
		result.message = "That door is already closed."
		return false
	elif obstacle.type != Obstacle.Type.DOOR_OPEN:
		result.message = "There is no door there to close."
		return false

	# Close the door
	obstacle.type = Obstacle.Type.DOOR_CLOSED
	result.message = "%s closed the door." % actor
	return true


func _to_string() -> String:
	return "CloseAction(actor: %s, direction: %s)" % [actor, direction]
